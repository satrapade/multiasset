...
take_profit<-function(
  ...
  carry_fwd_pnl<-strategy$pnl[carry_fwd_seq_along]
  period_pnl<-strategy$pnl-carry_fwd_pnl
  above_target<-(period_pnl>max_profit)
  ...
  data.table(
    ...
    period_target=period_above_target,
    stopped_pnl=stopped_pnl
  )
}
...
weighted_strategy<-function(
  ...
  backtest_matrix<-local({
    res<-do.call(cbind,mapply(function(b){
      if(stop_type=="SL")return(stop_loss(b,-abs(stop_level))$stopped_pnl)
      if(stop_type=="TP")return(take_profit(b,abs(stop_level))$stopped_pnl)
      b$pnl
...
compute_drawdown_stats<-function(strategy){
  ...
  data.table(
    res_stats[res],
    whole_sharpe=diff(strategy$pnl)%>%{16*mean(.)/sd(.)},
    whole_pnl=tail(strategy$pnl,1)
  )
...
make_sl_stats<-function(
  ...
  the_backtest<-backtest[date %in% dof$date]
  res<-mapply(
    function(level,backtest){
      dds<-compute_drawdown_stats(stop_loss(backtest,-abs(level))[,pnl:=stopped_pnl])
      data.table(
        levels=level,
