...
strategy1<-list(
  leg1=list(
    ...
    strike=function(close,vol,maturity){
      K_high<-D2PK(0.25,close,vol,maturity/365)
      P<-EP(close,K_high,maturity/365,0,vol)
      K_low<-P2K(K_high,P/3,close,vol,maturity/365)
      K_low
    },
    weight=3
  ),
  leg2=list(
    model=EP,
    strike=function(close,vol,maturity){
      K<-D2PK(0.25,close,vol,maturity/365)
      K
    },
    weight=-1
  )
...
g4<-melt(merge(
  x=make_strategy(
    strategy=list(
      leg1=list(
        model=EP,strike=function(close,vol,maturity)close*0.9,weight=1
      ),
      leg2=list(
        model=EP,strike=function(close,vol,maturity)close*1,weight=-1
      )
    ),
    shedule[market=="spx"],
    all_vol[market=="spx"]
...
\begin{center}
\Sexpr{make_plot(plot(g4),height="20cm",width="20cm")}
\end{center}
...
g5<-make_strategy(
  strategy=list(
      ...
      long_call=list(
        model=EC,
        strike=function(close,vol,maturity){
          K<-D2CK(0.40,close,vol,maturity/365)
          K
        },
        weight=1
      ),
      # paid for by shorting twice the puts, of lower delta
      short_put=list(
        model=EP,
        strike=function(close,vol,maturity){
          K_high<-D2CK(0.40,close,vol,maturity/365)
          C<-EC(close,K_high,maturity/365,0,vol)
          K_low<-P2K(close,C/2,close,vol,maturity/365)
          K_low
        },
        weight=-2
      )
...
ggtitle("1 40-delta call  funded by sellling 2 puts")
...
\begin{center}
\Sexpr{make_plot(plot(g5),height="20cm",width="20cm")}
\end{center}
