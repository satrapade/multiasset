...
\newpage
\section{Strategy, market backtests}

\begin{center}
\Sexpr{matrix2tabular(backtest_plot_matrix)}
\end{center}

<<,cache=FALSE, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, results="hide">>=

equal_backtest<-data.table(
  date=all_backtests$backtest[[1]]$date,
  pnl=rowMeans(do.call(cbind,mapply(function(b)b$pnl,all_backtests$backtest,SIMPLIFY=FALSE)))
)
...
@
\newpage
\section{All strategies, markets, equal weight}

\begin{center}
\Sexpr{make_plot(plot(g_equal),height="20cm",width="20cm")}
\end{center}

\newpage
\section{Portfolio 1 : Theoretical weights  }
...
backtest_matrix<-local({
    res<-do.call(cbind,mapply(function(b)b$pnl,all_backtests$backtest,SIMPLIFY=FALSE))
    colnames(res)<-paste0(all_backtests$market,"|",all_backtests$strategy)
    rownames(res)<-as.character(all_backtests$backtest[[1]]$date,format="%Y-%m-%d")
    res
  })
...
theoretical_backtest<-data.table(
  date=all_backtests$backtest[[1]]$date,
  pnl=local({
    drop(backtest_matrix %*% cbind(theoretical_weights[all_backtests$strategy] %>% {./sum(.)}))
  })
)
...
\begin{center}
\Sexpr{make_plot(plot(g_theoretical),height="20cm",width="20cm")}
\end{center}


\newpage
\section{Portfolio 2 : Weights matching average DOF exposure }
...
historical_backtest<-data.table(
  date=all_backtests$backtest[[1]]$date,
  pnl=local({
    drop(backtest_matrix %*% cbind(historical_weights[all_backtests$strategy] %>% {./sum(.)}))
  })
)
...
\begin{center}
\Sexpr{make_plot(plot(g_historical),height="20cm",width="20cm")}
\end{center}
...
spx_90_100_ps_backtest<-make_strategy(strategy=strategy_90_100_PS,shedule[market=="spx"],all_vol[market=="spx"])
