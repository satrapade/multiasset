...
make_sl_stats<-function(w)do.call(rbind,mapply(
...
make_sl_plot<-function(w,v=c("max_draw","median_draw")){
  res<-melt(
    make_sl_stats(w)[,c("levels",v),with=FALSE],
    id.vars = "levels",
    measure.vars = v
  ) 
  res %>%
  ggplot() +
  geom_line(aes(x=levels,y=value,col=variable))
}
