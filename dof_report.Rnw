...
\newpage
\section{Conclusions: Stop-loss vs return quality}
<<,cache=FALSE, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, results="hide">>=

#
#
equal_backtest_stop<-function(stop_level)data.table(
  date=all_backtests_3m$backtest[[1]]$date,
  pnl=rowMeans(do.call(cbind,mapply(function(b){
    stop_loss(b,-abs(stop_level))$stopped_pnl
  },all_backtests_3m$backtest,SIMPLIFY=FALSE))),
...
make_backtest_plot<-function(w)rbind(
...
#
#
make_drawdown_plot <- function(w,stop_level=0.025)rbind(
...
quality_vs_stop_df<-data.table(
  stop_loss=round(100*seq(0,0.2,length.out=25),digits=2),
  sharpe=round(sapply(
    seq(0,0.2,length.out=25),
    function(sl)equal_backtest_stop(sl)$pnl %>% {sd(diff(.))/mean(diff(.))/16}
  ),digits=2),
  calmar=round(sapply(
    seq(0,0.2,length.out=25),
    function(sl)equal_backtest_stop(sl)$pnl %>% {sum(diff(.))/max(cummax(.)-.)}
  ),digits=2),
  proxy="equal weight"
)
...
\begin{center}
\Sexpr{ntable(
  df=quality_vs_stop_df,
  title="Return quality vs Stop-loss level"
)}
\end{center}
...
equal_backtest<-weighted_strategy( weights=weights$equal, backtests=all_backtests_3m )
equal_backtest_drawdown_stats<-compute_drawdown_stats(equal_backtest)


g_equal_sl<-rbind(
  data.table(stop_loss(equal_backtest[date %in% dof$date],-0.01),stop="01pct"),
  data.table(stop_loss(equal_backtest[date %in% dof$date],-0.03),stop="03pct"),
  data.table(stop_loss(equal_backtest[date %in% dof$date],-0.05),stop="05pct"),
  data.table(stop_loss(equal_backtest[date %in% dof$date],-0.10),stop="10pct")
)%>%
ggplot() +
geom_line(aes(x=date,y=stopped_pnl,col=stop),size=2,alpha=0.75)+
ggtitle("Equal-weight all strategies, markets with stop-loss levels")

equal_backtest_drawdown_stats_stop<-compute_drawdown_stats(stop_loss(equal_backtest,-0.03)[,pnl:=stopped_pnl])

g_equal_drawdown_stats_stop <- rbind(
  data.table(dof_drawdown_stats[,.(date,drawdown)],strategy="dof"),
  data.table(equal_backtest_drawdown_stats_stop[date %in% dof$date,.(date,drawdown)],strategy="equal")
) %>%
ggplot() +
geom_line(aes(x=date,y=drawdown,col=strategy),size=2,alpha=0.75)+
  ggtitle("Equal-weight all strategies with stop-loss, markets vs DOF drawdowns")
...
\begin{center}
\begin{tabular}{m{10cm} m{10cm}}
\Sexpr{make_plot(plot(make_backtest_plot("equal")),height="10cm",width="10cm")}
&
\Sexpr{make_plot(plot(make_drawdown_plot("equal")),height="10cm",width="10cm")}
\\
\Sexpr{make_plot(plot(g_equal_sl),height="10cm",width="10cm")}
&
\Sexpr{make_plot(plot(g_equal_drawdown_stats_stop),height="10cm",width="10cm")}
\\
\end{tabular}
\end{center}
...
\noindent Note the effect of a 3\% stop-loss on drawdowns. The drawdown depth decreases, but drawdown length increases.
...
theoretical_backtest<-weighted_strategy( weights=weights$theoretical, backtests=all_backtests_3m)
theoretical_backtest_drawdown_stats<-compute_drawdown_stats(theoretical_backtest)

g_theoretical<-make_backtest_plot("theoretical")


g_theoretical_sl<-rbind(
  data.table(stop_loss(theoretical_backtest[date %in% dof$date],-0.01),stop="01pct"),
  data.table(stop_loss(theoretical_backtest[date %in% dof$date],-0.03),stop="03pct"),
  data.table(stop_loss(theoretical_backtest[date %in% dof$date],-0.05),stop="05pct"),
  data.table(stop_loss(theoretical_backtest[date %in% dof$date],-0.10),stop="10pct")
)%>%
ggplot() +
geom_line(aes(x=date,y=stopped_pnl,col=stop),size=2,alpha=0.75)+
ggtitle("Theoretical-weight all strategies, markets with stop-loss levels")
...
theoretical_backtest_drawdown_stats_stop<-compute_drawdown_stats(stop_loss(theoretical_backtest,-0.03)[,pnl:=stopped_pnl])

g_theoretical_drawdown_stats_stop <- rbind(
  data.table(dof_drawdown_stats[,.(date,drawdown)],strategy="dof"),
  data.table(theoretical_backtest_drawdown_stats_stop[date %in% dof$date,.(date,drawdown)],strategy="equal")
) %>%
ggplot() +
geom_line(aes(x=date,y=drawdown,col=strategy),size=2,alpha=0.75)+
  ggtitle("theoretical-weight all strategies with stop-loss, markets vs DOF drawdowns")
...
\begin{center}
\begin{tabular}{m{10cm} m{10cm}}
\Sexpr{make_plot(plot(make_backtest_plot("theoretical")),height="10cm",width="10cm")}
&
\Sexpr{make_plot(plot(make_drawdown_plot("theoretical")),height="10cm",width="10cm")}
\\
\Sexpr{make_plot(plot(g_theoretical_sl),height="10cm",width="10cm")}
&
\Sexpr{make_plot(plot(g_theoretical_drawdown_stats_stop),height="10cm",width="10cm")}
\\
\end{tabular}
\end{center}
...
historical_backtest<-weighted_strategy(weights=weights$historical, backtests=all_backtests_3m)
historical_backtest_drawdown_stats<-compute_drawdown_stats(historical_backtest)

g_historical<-make_backtest_plot("historical")

g_historical_sl<-rbind(
  data.table(stop_loss(historical_backtest[date %in% dof$date],-0.01),stop="01pct"),
  data.table(stop_loss(historical_backtest[date %in% dof$date],-0.03),stop="03pct"),
  data.table(stop_loss(historical_backtest[date %in% dof$date],-0.05),stop="05pct"),
  data.table(stop_loss(historical_backtest[date %in% dof$date],-0.10),stop="10pct")
)%>%
ggplot() +
geom_line(aes(x=date,y=stopped_pnl,col=stop),size=2,alpha=0.75)+
ggtitle("Historical-weight all strategies, markets with stop-loss levels")
...
historical_backtest_drawdown_stats_stop<-compute_drawdown_stats(stop_loss(historical_backtest,-0.03)[,pnl:=stopped_pnl])

g_historical_drawdown_stats_stop <- rbind(
  data.table(dof_drawdown_stats[,.(date,drawdown)],strategy="dof"),
  data.table(historical_backtest_drawdown_stats_stop[date %in% dof$date,.(date,drawdown)],strategy="equal")
) %>%
ggplot() +
geom_line(aes(x=date,y=drawdown,col=strategy),size=2,alpha=0.75)+
  ggtitle("historical-weight all strategies with stop-loss, markets vs DOF drawdowns")


stop_loss_scenarios<-rbind(
  compute_drawdown_stats(stop_loss(historical_backtest[date %in% dof$date],-0.10)[,pnl:=stopped_pnl])[
    max_draw==max(max_draw)
  ][1,.(
    episode,max_draw,length_draw
  )],
  compute_drawdown_stats(stop_loss(historical_backtest[date %in% dof$date],-0.05)[,pnl:=stopped_pnl])[
    max_draw==max(max_draw)
  ][1,.(
    episode,max_draw,length_draw
  )],
  compute_drawdown_stats(stop_loss(historical_backtest[date %in% dof$date],-0.03)[,pnl:=stopped_pnl])[
    max_draw==max(max_draw)
  ][1,.(
    episode,max_draw,length_draw
  )],
  compute_drawdown_stats(stop_loss(historical_backtest[date %in% dof$date],-0.001)[,pnl:=stopped_pnl])[
    max_draw==max(max_draw)
  ][1,.(
    episode,max_draw,length_draw
  )]
...
\begin{center}
\begin{tabular}{m{10cm} m{10cm}}
\Sexpr{make_plot(plot(make_backtest_plot("historical")),height="10cm",width="10cm")}
&
\Sexpr{make_plot(plot(make_drawdown_plot("historical")),height="10cm",width="10cm")}
\\
\Sexpr{make_plot(plot(g_historical_sl),height="10cm",width="10cm")}
&
\Sexpr{make_plot(plot(g_historical_drawdown_stats_stop),height="10cm",width="10cm")}
\\
\end{tabular}
\end{center}
...
\newpage
\section{A DOF proxy: SPX Q90-100 PS vs DOF performance}
...
g_spx_90_100_ps_sl<-rbind(
  data.table(stop_loss(spx_90_100_ps_backtest[date %in% dof$date],-0.01),stop="01pct"),
  data.table(stop_loss(spx_90_100_ps_backtest[date %in% dof$date],-0.03),stop="03pct"),
  data.table(stop_loss(spx_90_100_ps_backtest[date %in% dof$date],-0.05),stop="05pct"),
  data.table(stop_loss(spx_90_100_ps_backtest[date %in% dof$date],-0.10),stop="10pct")
)%>%
ggplot() +
geom_line(aes(x=date,y=stopped_pnl,col=stop),size=2,alpha=0.75)+
ggtitle("spx_90_100_ps with stop-loss levels")
...
spx_90_100_ps_drawdown_stats_stop<-compute_drawdown_stats(stop_loss(spx_90_100_ps_backtest,-0.03)[,pnl:=stopped_pnl])

g_historical_drawdown_stats_stop <- rbind(
  data.table(dof_drawdown_stats[,.(date,drawdown)],strategy="dof"),
  data.table(spx_90_100_ps_drawdown_stats_stop[date %in% dof$date,.(date,drawdown)],strategy="equal")
) %>%
ggplot() +
geom_line(aes(x=date,y=drawdown,col=strategy),size=2,alpha=0.75)+
  ggtitle("SPX 90-100 put-spread with stop-loss, markets vs DOF drawdowns")
...
\\
\Sexpr{make_plot(plot(g_spx_90_100_ps_sl),height="10cm",width="10cm")}
&
\Sexpr{make_plot(plot(g_historical_drawdown_stats_stop),height="10cm",width="10cm")}
\\
\end{tabular}
