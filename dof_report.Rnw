...
backtest2<-make_backtest(
  shedule=shedule[market=="spx"],
  volsurf=all_vol[market=="spx"],
  model=EP,
  strike_fun=function(close,vol,maturity)close*exp(qnorm(0.15,sd=vol*sqrt(maturity/365),lower.tail=TRUE))
)

backtest3<-make_backtest(
  shedule=shedule[market=="spx"],
  volsurf=all_vol[market=="spx"],
  model=EP,
  strike_fun=function(close,vol,maturity)close*exp(qnorm(0.5,sd=vol*sqrt(maturity/365),lower.tail=TRUE))
)

backtest<-data.table(
  date=backtest3$date,
  pnl=backtest2$pnl-backtest3$pnl
)
