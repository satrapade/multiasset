...
make_backtest<-function(shedule,volsurf,model)
{
  options<-merge(
    x=shedule,
    y=compute_option_premium(
...
x<-make_backtest(shedule[market=="spx"],all_vol[market=="spx"],EP)
...
backtest2<-merge(
  ...
  y=compute_option_premium(
    ...
    strike_fun=function(close,vol,maturity)close*0.9,
    model=EP
  )[,.(market,date,strike_close,strike,option_maturity,option_vol,premium)],
  by.x=c("date","market"),
  by.y=c("date","market")
)
...
system.time(backtest3<-merge(
  ...
  y=compute_option_premium(
    shedule[market=="spx"],
    all_vol[market=="spx"],
    strike_fun=function(close,vol,maturity)close*1.0,
    model=EP
  )[,.(market,date,strike,strike_vol,premium)],
  by.x=c("date","market"),
  by.y=c("date","market")
...
backtest<-data.table(
  ...
  maturity=backtest2$maturity,
  intrinsic=(backtest2$strike-backtest3$strike)/backtest3$strike,
  reval=(backtest2$premium-backtest3$premium)/backtest3$strike
)[,c(.SD,list(
  cash=cumsum(ifelse(
    start==date,
    -reval,
...
x<-melt(merge(x=backtest,y=dof,by.x="date",by.y="Date")[,.(
  date,
  backtest=cumsum(c(0,diff(pnl)))*mean(diff(PnL))/mean(diff(pnl)),
  dof=cumsum(c(0,diff(PnL)))
)],id.vars = "date",measure.vars = c("backtest","dof"))

y<-apply(do.call(cbind,split(x$value,x$variable)),2,diff)

cm<-cor(apply(y,2,roll_mean,14))
