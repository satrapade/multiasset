...
\noindent We often need to modify a vector by replacing some of its elements with preceding values, based on
some criterion. There are 2 ways to accomplish this:
...
\newpage
\section{Resetting sum, version 1}
...
Because of frequent use, a vectorized calculation of the {\bf re-setting sum} is needed. One way to do this is to 
to 
...
\newpage
\section{Resetting sum, version 2}
...
\newpage
\section{All combinations of 2 vectors using {\tt rep}}
...
\newpage
\section{Computing a sorting permutation matrix from ranks}

\vskip 5mm

\noindent It is sometimes useful to express sorting operations in the form of permutation matrices. We use dense element
ranks as column indices into the diagonal matrix to form the permutation matrix.
